...
async def test_agent_can_query_a_host_provided_connection_with_a_row_limit() -> None:
...
async def test_agent_cannot_run_unsafe_sql(sql: str, expected_error: str) -> None:
    connection = duckdb.connect(":memory:", config={"enable_external_access": "false"})
    connection.execute("create table numbers(value integer)")
...
async def test_connection_must_disable_external_access() -> None:
...
async def test_parallel_tool_calls_share_one_host_connection_safely() -> None:
    connection = duckdb.connect(":memory:", config={"enable_external_access": "false"})
